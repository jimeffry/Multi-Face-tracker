...
class FaceDetector(object):
    def __init__(self,prefix,file_in='None',min_size=50,score_threshold=[0.5,0.7,0.9],\
                batch_size=[1,256,32],epoch_load=[32,30,25],nms_thresh=[0.3,0.3,0.3],test_relu=1):
        self.test_relu = test_relu
    ...
    def load_model(self):
        if self.test_relu==5 or self.test_relu==100:
            if config.rnet_wide:
                #5,500,60;  5,1700,60
            ...
            #[205,500,200]
            #prefix = ["../../models/MTCNN_bright_model/PNet_landmark/PNet", "../../models/MTCNN_bright_model/RNet_landmark/RNet", "../../models/MTCNN_bright_model/ONet_landmark/ONet"]
            ...
            prefix = self.prefix
        print("demo epoch load ",self.epoch_load)
        model_path = ["%s-%s" %(x,y ) for x, y in zip(prefix,self.epoch_load)]
        print("demo model path ",model_path)
        #param = parameter()
        min_size = self.min_size
        score_threshold = self.score_threshold
        if self.test_relu==100:
            batch_size = [1,1,1]
        else:
            batch_size = self.batch_size
        epoch_load = self.epoch_load
        multi_detector = [None,None,None]
        #load net result
        print("model path0 ",model_path[0])
        ...
        Mtcnn_detector = MtcnnDetector(multi_detector,min_size,threshold=score_threshold,nms_thresh=self.nms_thresh)
        #bboxs,bbox_clib,landmarks = Mtcnn_detector.detect(img)
        return Mtcnn_detector

    def add_label(self,img,bbox,landmark):
    ...
    def camera(self):
        ...
        if self.file_in =='None':
            ...
            camera_cap = cv2.VideoCapture(self.file_in)
        if not camera_cap.isOpened():
            print("failded open camera")
            return -1
        mtcnn_dec = self.detector
        while camera_cap.isOpened():
            ...
            if ret:
                ...
                if len(bbox_clib):
                    #bbox_clib = self.board_img(bbox_clib,w,h)
                    self.add_label(frame,bbox_clib,landmarks)
    ...
    def demo_img(self):
        ...
        if len(bbox_clib):
            self.add_label(img,bbox_clib,landmarks)
            cv2.imshow("result",img)
            cv2.waitKey(0)

    def get_face(self,img):
        bbox_clib,landmarks = self.detector.detect(img)
        h,w = img.shape[:2]
        if len(bbox_clib)>0:
            #bbox_clib = np.array(bbox_clib)
            bbox_clib = self.board_img(bbox_clib,w,h)
    ...
    def board_img(self,boxes,wid,height):
...
if __name__ == '__main__':
    prefix = ["../../models/MTCNN_bright_model/PNet_landmark/PNet", "../../models/MTCNN_bright_model/RNet_landmark/RNet", "../../models/MTCNN_bright_model/ONet_landmark/ONet"]
    #file_in="/home/lxy/Downloads/DataSet/videos/subway_gate2.avi"
    #face_detect = FaceDetector(prefix,file_in)
    face_detect = FaceDetector(prefix)
    face_detect.camera()
